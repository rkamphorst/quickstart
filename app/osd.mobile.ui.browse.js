...
(function($) {
    ...
    // objects (nodes?) that are searched for params
    var findObjects = [];
     ...
     */
    function setTemplate (viewKey, templateUrl) {
     ...
     */
    function setTemplates (map) {
        for (var viewKey in map) {
            if (map.hasOwnProperty(viewKey)) {
                if (typeof map[viewKey] == "string") {
                    setTemplate(viewKey, map[viewKey]);
                }
            }
        }
    };
     ...
     */
    function getHashUrl (object, extParams, viewKey) {
    ...
    function doTransition (params) {
        ...
        var loadingTimeoutId = window.setTimeout(showLoading, 100);
